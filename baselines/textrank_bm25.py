    ...
    def rum_textrank_bm25(self, query, doc_tensors, tf_dict, ranker, mention):
        ...
        for dt in doc_tensors:
            if len(dt) > 800:
                ...
                avg_doc_len += len(dt)
        avg_doc_len /= len(doc_tensors)
        for num, doc in enumerate(doc_tensors):
            qtf = torch.FloatTensor([tf_dict[num].get(term, 0) for term in query])
            score_list.append(ranker(qtf, idf, doc, avg_doc_len))
...
class _bm25(nn.Module):
    ...
    def forward(self, qtf, idf, d, ave_dl, without_idf = False):
        num = qtf * (self.k1 + 1)
        denom = qtf + self.k1 * (1 - self.b + self.b * len(d) / ave_dl)
