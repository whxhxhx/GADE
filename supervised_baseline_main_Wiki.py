...
def test_sbert(iter, logger, lrm, criterion, test_step=None, prefix='Test'):
    lrm.eval()
...
def train_sbert(iter, checkpoint_path, logger, fold, lrm, optimizer, criterion, epoch_num,
    ...
    for i in range(start_epoch, epoch_num):
        ...
        if val_iter:
            f1_score = test_sbert(iter=val_iter, logger=logger, lrm=lrm, prefix='Val',
    ...
    if test_iter:
        ...
        f1_score = test_sbert(iter=test_iter, logger=logger, lrm=lrm, prefix='Test',
                      criterion=criterion, test_step=i + 1)
        logger.info("Test F1 score\tEpoch\t{:d}\t{:.4f}".format(best_epoch, f1_score))
        f1_list.append(f1_score)


def test_dpr(iter, logger, lrm, criterion, test_step=None, prefix='Test'):
    lrm.eval()
    ...
    for j, batch in enumerate(iter):
        with torch.no_grad():
            ...
            pred = pred[masks == 1]
            loss = criterion(pred, label)
            p, r, acc = accuracy_dpr(pred, label)
            logger.info(
                '{}\t[{:d}/{:d}]\tLoss {:.3f}\tAccuracy {:.3f}\tPrecison {:.3f}\tRecall {:.3f}'.format(prefix, j + 1,
                                                                                                       len(iter), loss,
                                                                                                       acc,
    ...
    p, r, f1, acc = calculate_f1_dpr(scores, labels)
    logger.info(
        '{}\tPrecison {:.3f}\tRecall {:.3f}\tF1-score {:.3f}\tAccuracy {:.3f}'.format(prefix, p, r, f1, acc))
...
def train_dpr(iter, checkpoint_path, logger, fold, lrm, optimizer, criterion, epoch_num,
          start_epoch=0, scheduler=None, test_iter=None, val_iter=None, log_freq=1, start_f1=None):
    ...
    for i in range(start_epoch, epoch_num):
        ...
        if val_iter:
            f1_score = test_dpr(iter=val_iter, logger=logger, lrm=lrm, prefix='Val',
                      criterion=criterion, test_step=i + 1)
            if f1_score > best_f1:
                best_f1 = f1_score
                state = {
    ...
    if test_iter:
        ...
        val_f1 = checkpoint["val_f1"]
        logger.info("load from epoch {:d}  f1 score {:.4f}".format(best_epoch, val_f1))
        f1_score = test_dpr(iter=test_iter, logger=logger, lrm=lrm, prefix='Test',
                      criterion=criterion, test_step=i + 1)
        logger.info("Test F1 score\tEpoch\t{:d}\t{:.4f}".format(best_epoch, f1_score))
        f1_list.append(f1_score)
...
if __name__ == '__main__':
    ...
    for i in range(kfold):
        ...
        if args.model_name in ['sbert_100', 'sbert_200', 'sbert_300']:
            criterion = nn.CrossEntropyLoss().to(device)
            train_sbert(train_iter, checkpoint_path, logger, i, lrm, opt, criterion, args.epochs, test_iter=test_iter,
                  val_iter=val_iter, scheduler=scheduler, log_freq=args.log_freq, start_epoch=start_epoch, start_f1=start_f1)
        elif args.model_name in ['DPR_100', 'DPR_200', 'DPR_300']:
            criterion = nn.BCELoss().to(device)
            train_dpr(train_iter, checkpoint_path, logger, i, lrm, opt, criterion, args.epochs, test_iter=test_iter,
                        val_iter=val_iter, scheduler=scheduler, log_freq=args.log_freq, start_epoch=start_epoch,
                        start_f1=start_f1)
        else:
